...
import { LITE_LOCATION_ID, LITE_TEAM_ID } from "../lite/contracts.js";
import {
  ...
  type ProtectedMessageContentDocument,
...
/**
 * Governed Meta WhatsApp canary lane. Signed provider receipts and downstream
 * effects remain content-free. Newly accepted allowlisted bot text may also be
 * retained separately, without a sender identifier, in the short-lived
 * backend-only protected inbox collection.
 */
...
export type ProtectedInboundPersistencePlan = {
  readonly conversationId: string;
  readonly contentId: string;
  readonly messageId: string;
  readonly document: ProtectedMessageContentDocument;
};

/**
 * Treat a private STOP/UNSUBSCRIBE as sender-wide for the complete signed
 * envelope. This prevents another message from the same sender in that batch
 * from being retained simply because it has a different provider message id.
 * Public opt-outs are handled by the separate public suppression lane.
 */
export function shouldSuppressProtectedInboundForPrivateOptOut(input: {
  readonly message: CanaryBotMessage | undefined;
  ...
  return input.optOuts.some((optOut) =>
    !input.publicInboundMessageIds.has(optOut.waMessageId) &&
    optOut.waId === input.message?.waId
  );
}

/**
 * Produce a protected-content write only for the exact governed inbound lane.
 * The caller supplies a message selected by the allowlist extractor; all
 * public, replayed, opted-out, non-bot, and non-replying records fail closed.
 */
export function buildProtectedInboundPersistencePlan(input: {
  readonly record: CanaryInboundRecord;
  readonly allowlistedMessage: CanaryBotMessage | undefined;
  readonly receiptPlanKind: "create" | "replay";
  readonly planMode: "bot" | "auto_reply" | "none";
  readonly hasBotPlanEffect: boolean;
  readonly publicInbound: boolean;
  readonly optedOut: boolean;
  readonly occurredAtMs: number;
  readonly retentionDays?: unknown;
}): ProtectedInboundPersistencePlan | null {
  const message = input.allowlistedMessage;
  if (
    input.record.kind !== "message" ||
    input.receiptPlanKind !== "create" ||
    input.planMode !== "bot" ||
    !input.hasBotPlanEffect ||
    input.publicInbound ||
    input.optedOut ||
    !message ||
    message.waMessageId !== input.record.waMessageId
  ) {
    return null;
  }

  const content = extractProtectedInboundMessageContent(message);
  if (!content) return null;

  const { conversationId } = liveIds(message.waId);
  const contentId = protectedInboundContentId(input.record.waMessageId);
  const messageId = `message_live_${sha256Hex(
    `${input.record.waMessageId}:inbound`,
  ).slice(0, 16)}`;
  return {
    conversationId,
    contentId,
    messageId,
    document: buildProtectedMessageContentDocument({
      id: contentId,
...
export async function persistCanaryWebhookBatch(input: {
  ...
  const allowlistedMessagesById = new Map(
    input.allowlistedMessages.map((message) => [message.waMessageId, message] as const),
  );
    ...
    for (const record of recordsForPlan) {
      ...
      if (plan.kind === "create") {
        ...
        if (record.kind === "message") {
          ...
          const allowlistedMessage = allowlistedMessagesById.get(
            record.waMessageId,
          );
          const protectedPlan = buildProtectedInboundPersistencePlan({
            record,
            allowlistedMessage,
            receiptPlanKind: "create",
            planMode,
            hasBotPlanEffect: plan.effectIds.some(
              (effectId) =>
                plannedEffectsById.get(effectId)?.effectKind === "bot_plan",
            ),
            publicInbound: isPublicReceipt,
            optedOut: shouldSuppressProtectedInboundForPrivateOptOut({
              message: allowlistedMessage,
              optOuts: input.optOuts,
              publicInboundMessageIds: input.publicInboundMessageIds,
            }),
          ...
          if (protectedPlan) {
            transaction.create(
              ws.collection("conversations")
                .doc(protectedPlan.conversationId)
                .collection(LITE_PROTECTED_MESSAGE_CONTENTS_COLLECTION)
                .doc(protectedPlan.contentId),
              protectedPlan.document,
            );
          }
        }
 ...
 * stage does not persist or application-log raw text or generated answers.
 * The send boundary may reserve an exact private-canary reply in protected
 * short-retention storage; failures retain the deterministic menu fallback.
 */
...
export const metaCanaryWebhook = onRequest(
  ...
  async (request, response) => {
    ...
    logger.info("meta-canary: durable webhook accepted", {
