 ...
 * - Reduce inbound WhatsApp payloads to CONTENT-FREE provider receipts.
 *   Eligible private canary text may be retained separately under the
 *   protected-inbox policy; sender numbers are never stored verbatim.
...
export type CanaryBotMessage = {
  ...
  /** Full bounded text for protected retention only; never durable bot input. */
 ...
 * explicit tester allowlist. Full sender digits remain transient request-memory
 * values. Only the separately governed protected-inbox writer may persist the
 * bounded `protectedText`; no caller may log either value.
 */
      ...
      for (const messageValue of messages) {
        ...
        const mediaRecord =
          messageType === "image" ||
            ...
            ? asRecord(message[messageType])
            : null;
        const mediaCaption = typeof mediaRecord?.caption === "string"
          ? mediaRecord.caption
          : "";
        const protectedText = (textBody || mediaCaption || selectionTitle).slice(0, 4_096);
        const text = textBody.slice(0, 512);
        out.push({
          waId,
          waMessageId,
          messageType,
