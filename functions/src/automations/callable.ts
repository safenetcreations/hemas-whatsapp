...
function getDemoFirestore(boundary: ReturnType<typeof emulatorBoundary>): Firestore {
  ...
  if (app.options.projectId && app.options.projectId !== boundary.projectId) {
    throw new FailClosedError("invalid_audit_project", "The Admin app does not match the synthetic demo project.");
  }
...
function callableError(error: unknown): HttpsError {
  if (error instanceof ConfigValidationError) return new HttpsError("failed-precondition", "The synthetic automation runtime is not ready.");
  if (!(error instanceof FailClosedError)) return new HttpsError("internal", "The synthetic automation service failed closed.");
  if (error.code === "authentication_required") return new HttpsError("unauthenticated", error.message);
  if (error.code === "automation_revision_conflict") return new HttpsError("aborted", error.message);
  ...
  if (error.code.includes("schedule") || error.code.includes("transition") || error.code === "recent_auth_required") {
    return new HttpsError("failed-precondition", error.message);
  }
  return new HttpsError("permission-denied", error.message);
}

export const demoControlSyntheticAutomationRun = onCall(
  ...
  async (request) => {
    try {
      ...
      const parsed = parseSyntheticAutomationControlInput(request.data);
      return await controlSyntheticAutomationRun({
