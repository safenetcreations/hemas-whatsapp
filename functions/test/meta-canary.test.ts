...
import {
  canaryPortalMessageProjectionTarget,
...
import {
  buildProtectedInboundPersistencePlan,
  isMetaCanaryPublicInboundWindowOpen,
  metaCanaryPublicProviderOccurredAtMs,
  shouldSuppressProtectedInboundForPrivateOptOut,
} from "../src/meta-canary/index.js";
import { protectedBotEffectContentId } from "../src/lite/protected-messages.js";
...
test("bot extraction admits only allowlisted inbound messages and dedupes provider ids", () => {
  ...
  assert.equal(messages[1]?.protectedText, "Book appointment");
...
test("protected inbound retains the bounded full text while bot routing stays at 512 characters", () => {
  ...
  const allowlist = parseRecipientAllowlist("+94771234567");
  const message = extractAllowlistedCanaryBotMessages(payload, allowlist)[0];
  const record = extractCanaryInboundRecords(payload, sha)[0];
  assert.ok(message);
  assert.ok(record);
  assert.equal(message.inbound.text.length, 512);
  assert.equal(message.protectedText, body);

  const plan = buildProtectedInboundPersistencePlan({
    record,
    allowlistedMessage: message,
    receiptPlanKind: "create",
    planMode: "bot",
    hasBotPlanEffect: true,
    publicInbound: false,
    optedOut: false,
    occurredAtMs: 1_800_000_000_000,
    retentionDays: 7,
  });
  assert.equal(plan?.document.text, body);
  assert.equal(plan?.document.bodyLength, body.length);
});

test("a private opt-out suppresses protected content for every same-sender message in the envelope", () => {
  const message = {
    waId: "94771234567",
    waMessageId: "wamid.same-sender-question",
    messageType: "text",
    protectedText: "Please show the menu",
    inbound: { kind: "text", selectionId: "", text: "Please show the menu" },
  } as const;
  const privateStop = {
    waId: message.waId,
    waMessageId: "wamid.same-sender-stop",
    keyword: "STOP" as const,
  };
  assert.equal(shouldSuppressProtectedInboundForPrivateOptOut({
    message,
    optOuts: [privateStop],
  ...
  assert.equal(shouldSuppressProtectedInboundForPrivateOptOut({
    message,
    optOuts: [{ ...privateStop, waId: "94770000000" }],
    publicInboundMessageIds: new Set(),
  }), false);
  assert.equal(shouldSuppressProtectedInboundForPrivateOptOut({
    message,
    optOuts: [privateStop],
    publicInboundMessageIds: new Set([privateStop.waMessageId]),
  }), false);
});

test("protected inbound content is materialized only for a new private allowlisted bot reply", () => {
  const occurredAtMs = 1_800_000_000_000;
  const waId = "94771234567";
  const payload = {
    object: "whatsapp_business_account",
    entry: [{
      changes: [{
        ...
        value: {
          ...
          messages: [
            {
              id: "wamid.protected-text",
              from: waId,
              type: "text",
              text: { body: "  Please show the appointment menu  " },
            },
            {
              id: "wamid.protected-selection",
              from: waId,
              type: "interactive",
              interactive: {
                button_reply: {
            ...
            {
              id: "wamid.protected-caption",
              from: waId,
              type: "image",
              image: {
                id: "provider-media-id-must-not-persist",
                caption: "  Appointment card caption  ",
              },
            },
            {
              id: "wamid.protected-stop",
              from: waId,
              type: "text",
              text: { body: "STOP" },
            },
  ...
  const allowlist = parseRecipientAllowlist(`+${waId}`);
  ...
  const recordByMessageId = new Map(
    records.map((record) => [record.waMessageId, record] as const),
  );
  const messageById = new Map(
    messages.map((message) => [message.waMessageId, message] as const),
  );

  const textRecord = recordByMessageId.get("wamid.protected-text")!;
  const eligible = buildProtectedInboundPersistencePlan({
    record: textRecord,
    allowlistedMessage: messageById.get(textRecord.waMessageId),
    receiptPlanKind: "create",
    planMode: "bot",
    hasBotPlanEffect: true,
    publicInbound: false,
  ...
  assert.equal(
    ...
    `conversation_live_${sha(`live:${waId}`).slice(0, 10)}`,
  );
  assert.equal(
    eligible.messageId,
    `message_live_${sha(`${textRecord.waMessageId}:inbound`).slice(0, 16)}`,
  );
  assert.equal(eligible.document.id, eligible.contentId);
  assert.equal(eligible.document.messageId, eligible.messageId);
  assert.equal(eligible.document.direction, "inbound");
  assert.equal(eligible.document.source, "meta_inbound");
  assert.equal(eligible.document.contentKind, "text");
  assert.equal(eligible.document.text, "Please show the appointment menu");
  assert.equal(eligible.document.state, "materialized");
  assert.equal(eligible.document.containsMessageContent, true);
  assert.equal(eligible.document.containsPlaintextSender, false);
  assert.equal(eligible.document.createdAt.toMillis(), occurredAtMs);
  assert.equal(
    eligible.document.expiresAtMs,
    occurredAtMs + 7 * 24 * 60 * 60 * 1_000,
  );
  const serialized = JSON.stringify(eligible.document);
  assert.equal(serialized.includes(waId), false);
  assert.equal(serialized.includes("mediaId"), false);
  assert.equal(serialized.includes("sender"), false);

  const selectionRecord = recordByMessageId.get("wamid.protected-selection")!;
  const selection = buildProtectedInboundPersistencePlan({
    record: selectionRecord,
    allowlistedMessage: messageById.get(selectionRecord.waMessageId),
    receiptPlanKind: "create",
    planMode: "bot",
    hasBotPlanEffect: true,
    publicInbound: false,
  ...
  assert.equal(selection?.document.contentKind, "selection");
  assert.equal(selection?.document.text, "Book appointment");

  const captionRecord = recordByMessageId.get("wamid.protected-caption")!;
  const caption = buildProtectedInboundPersistencePlan({
    record: captionRecord,
    allowlistedMessage: messageById.get(captionRecord.waMessageId),
    receiptPlanKind: "create",
    planMode: "bot",
    hasBotPlanEffect: true,
    publicInbound: false,
  ...
  assert.equal(caption?.document.contentKind, "media_caption");
  assert.equal(caption?.document.text, "Appointment card caption");
  assert.equal(
    JSON.stringify(caption?.document).includes("provider-media-id-must-not-persist"),
    false,
  );
  ...
  const ineligibleBase = {
    record: textRecord,
    allowlistedMessage: messageById.get(textRecord.waMessageId),
    receiptPlanKind: "create" as const,
  ...
  assert.equal(buildProtectedInboundPersistencePlan({
    ...ineligibleBase,
    receiptPlanKind: "replay",
  }), null);
  assert.equal(buildProtectedInboundPersistencePlan({
    ...ineligibleBase,
    planMode: "auto_reply",
  ...
  assert.equal(buildProtectedInboundPersistencePlan({
    ...ineligibleBase,
    allowlistedMessage: undefined,
  }), null);

  const stopRecord = recordByMessageId.get("wamid.protected-stop")!;
  assert.equal(optOuts.some((optOut) => optOut.waMessageId === stopRecord.waMessageId), true);
  assert.equal(buildProtectedInboundPersistencePlan({
    ...ineligibleBase,
    record: stopRecord,
    allowlistedMessage: messageById.get(stopRecord.waMessageId),
    optedOut: true,
  }), null, "STOP must never create protected inbox content");
});
...
test("durable planner creates deterministic per-effect ids and STOP blocks bot side effects", () => {
  ...
  assert.deepEqual(
    ...
    ["inbound_metric"],
    "a same-envelope STOP suppresses every automation candidate from that sender",
  );
...
test("transient AI replies are content-free at rest and recover to the durable fallback", () => {
  ...
  const aiDispatch = markCanaryOutboxDispatchStarted(
    ...
    "ai_transient",
    {
      ...
      contentId: protectedBotEffectContentId(aiClaim.record.id),
    },
  );
  assert.equal(aiDispatch.dispatchReplySource, "ai_transient");
  assert.equal(
    aiDispatch.protectedConversationId,
    "conversation_live_0123456789",
  );
  assert.equal(
    aiDispatch.protectedContentId,
    protectedBotEffectContentId(aiClaim.record.id),
  );
  assert.deepEqual(parseCanaryOutboxRecord(aiDispatch), aiDispatch);
  assert.equal(parseCanaryOutboxRecord({
    ...aiDispatch,
    protectedContentId: undefined,
  }), null, "a protected dispatch locator is all-or-nothing");
  const ambiguousAiDispatch = failCanaryOutboxRecord({
    record: aiDispatch,
    leaseToken: "ai-source-lease",
    nowMs: 1_300,
  ...
  assert.equal(ambiguousAiDispatch.state, "fatal");
  assert.equal(
    ambiguousAiDispatch.protectedConversationId,
    aiDispatch.protectedConversationId,
  );
  assert.equal(
    ambiguousAiDispatch.protectedContentId,
    aiDispatch.protectedContentId,
    "fatal evidence keeps the content-free locator after allowlist rotation",
  );
  assert.deepEqual(
    parseCanaryOutboxRecord(ambiguousAiDispatch),
    ambiguousAiDispatch,
  );
...
test("delivery routes resolve to content-free bot and agent timeline records", () => {
  const providerMessageRef = "wamid.delivery-projection";
  const pending = buildCanaryOutboxRecord({
    id: canaryOutboxEffectId(
      "wamid.delivery-projection-inbound",
      "graph_bot_reply",
      "reply-0",
      sha,
    ),
    receiptId: "wamid.delivery-projection-inbound",
    effectKind: "graph_bot_reply",
    payload: {
      providerMessageRef: "wamid.delivery-projection-inbound",
      replyIndex: 0,
  ...
  const claim = claimCanaryOutboxRecord({
    record: pending,
    nowMs: 2_000,
    leaseToken: "delivery-projection",
    leaseOwner: "http_webhook",
  });
  assert.equal(claim.action, "claimed");
  if (claim.action !== "claimed") return;
  ...
  const botTarget = canaryPortalMessageProjectionTarget({
    targetKind: "meta_outbox_effect",
    targetId: completed.id,
    target: completed,
    providerMessageRef,
  });
  assert.ok(botTarget);
  assert.equal(botTarget.automationSource, "governed_ai");
  assert.equal(botTarget.agentReply, false);
  assert.equal(
    isCanaryPortalMessageProjectionValid({
      target: botTarget,
      message: {
        id: botTarget.messageId,
        workspaceId: "workspace_safenet_demo",
        direction: "outbound",
  ...
  assert.equal(
    isCanaryPortalMessageProjectionValid({
      ...
      message: {
        ...
        synthetic: true,
        liveCanary: true,
        automationSource: "menu_bot",
      },
    }),
    false,
  );

  const agentTarget = canaryPortalMessageProjectionTarget({
    targetKind: "lite_agent_reply",
    targetId: "reply_1234567890abcdef",
    target: {},
    providerMessageRef,
  });
  assert.ok(agentTarget);
  assert.equal(agentTarget.agentReply, true);
  assert.equal(agentTarget.automationSource, undefined);
  assert.equal(
    isCanaryPortalMessageProjectionValid({
      target: agentTarget,
      message: {
        id: agentTarget.messageId,
        workspaceId: "workspace_safenet_demo",
        direction: "outbound",
        externalDispatch: "dispatched",
