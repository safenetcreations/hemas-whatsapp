...
function actionFailureMessage(error: unknown, action: SyntheticAppointmentAction): string {
  ...
  if (code === "identity_mismatch" || code === "authentication_required") {
    return `The verified Firebase Auth identity could not authorize this ${label} request. Sign in to the demo account again.`;
  }
  ...
  if (code === "emulator_unavailable") {
    return `${FUNCTIONS_SOURCE.charAt(0).toUpperCase()}${FUNCTIONS_SOURCE.slice(1)} could not process the ${label} request. No fallback was attempted.`;
  }
  ...
  if (code === "unsafe_endpoint") {
    return `The ${label} request was blocked because the endpoint was not the approved governed Functions endpoint.`;
  }
      ...
      try {
        ...
        if (!reloaded) {
          setActionState({
            ...
            appointmentId,
            message:
              "The request returned, but the authoritative reload failed. Use Retry before relying on its state.",
          });
          return;
        }
        setActionState(
          ...
          setActionState({
            ...
            message: reloaded
              ? error.code === "revision_conflict"
                ? "Another audited transaction changed this simulator appointment. Authoritative Firestore state was reloaded; review it before trying again."
                : "The deterministic request identity conflicted with durable evidence. Authoritative Firestore state was reloaded and no new transition was assumed."
              : "A callable conflict occurred and the authoritative Firestore state could not be reloaded. Use Retry.",
          });
        } else {
          setActionState({
            status: "error",
