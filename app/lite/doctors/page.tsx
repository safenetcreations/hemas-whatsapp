  ...
  return (
    <div className="space-y-5">
      ...
      <section className="rounded-2xl border border-emerald-900/5 bg-white p-5 shadow-sm">
        ...
        {notice ? <p className="mt-3 rounded-lg bg-emerald-50 px-3 py-2 text-xs text-emerald-800">{notice}</p> : null}
      </section>
      ...
      <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {doctors.rows.map((d) => (
          <article key={d.id} className={`rounded-2xl border bg-white p-4 shadow-sm ${d.active ? "border-emerald-900/5" : "border-slate-200 opacity-60"}`}>
